...
function datePickerUpdateData(start,end) {
    var appElement = document.querySelector('[ng-controller=analyticsTransactionInfoCtrl]');
    var $scope = angular.element(appElement).scope();
    $scope.replaceModel(start,end);
}

var analyticsApp = angular.module('analyticsTransactionInfoApp', []);
analyticsApp.controller('analyticsTransactionInfoCtrl', function ($scope, $http, $location) {
    ...
    $scope.replaceModel = function (start,end) {
        ...
        $scope.appUrl= dataSourceURL()+"/transactionHealthEvents.pinpoint?appName="+ $scope.currentAppName+"&from="
            +$scope.selectedStartTime +"&to="+$scope.selectedEndTime;
        if($scope.isMockFlag){
            $scope.appUrl = "../mockjson/analyticsTransactionHealthEvents_"+$scope.currentAppName + ".json";
        }
    ...
    $scope.loadAppData = function (response) {
        ...
        $scope.currentAppName = response.appName;
        $scope.appTInfos = response.transactionInfoList;
    }
